...
/**
 * Class WC_Gateway_NicheClear_Generic
 *
 * This class represents a generic payment gateway in WooCommerce for the NicheClear payment processors.
 */
class WC_Gateway_NicheClear_Generic extends WC_Gateway_NicheClear_Base {
	...
	public function __construct($payment_processor_code) {
		...
		if ( $this->get_option( 'sandbox' ) == 'yes' ) {
			...
			$this->api_key    = $settings['api_key_sandbox'] ?? null;
		} else {
			$this->is_sandbox = false;
			$this->api_key    = $settings['api_key_prod'] ?? null;
		}
